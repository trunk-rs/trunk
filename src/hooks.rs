    ...
    let futures: FuturesUnordered<_> = cfg
        ...
        .map(|hook_cfg| {
            ...
            tokio::spawn(async move {
                let status = command
                    .spawn()
                    .with_reason(|| ErrorReason::HookCommandFailed { command: command_name.clone() })?
                    .wait()
                    .await
                .with_reason(|| ErrorReason::HookCommandFailed { command: command_name.clone() })?;
                if !status.success() {
                    return Err(ErrorReason::HookCommandFailed { command: command_name }.into_error());
                }
...
pub async fn wait_hooks(mut futures: HookHandles) -> Result<()> {
    while let Some(result) = futures.next().await {
        result.reason(ErrorReason::TokioTaskFailed)??;
    }
